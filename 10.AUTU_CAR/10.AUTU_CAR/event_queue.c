...
t_Event eventQueue[QUEUE_SIZE];  // 이벤트 저장 공간
...
uint8_t push_event_queue(t_Event inEvent) {
	...
	ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
		...
		++eventQCount;
		//printf("push event count : %d  rear : %d\r\n", eventQCount, eventQRear);
	}
	return EQ_SUCCESS;
}
...
uint8_t pop_event_queue(t_Event **ppEvent) {
	...
	ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
		...
		*ppEvent = &eventQueue[eventQFront++];
		...
		--eventQCount;
		//printf("after pop event count : %d  front : %d\r\n", eventQCount, eventQFront);
	}
